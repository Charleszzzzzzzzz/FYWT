...
// Interface for the data points used in the Histogram chart
interface DataPointForHistogram {
  name: string; // Corresponds to '时间'
  value: number; // Corresponds to '涨跌幅(%)'
}

// Interface for the third argument provided by Recharts Tooltip's formatter function
// This aims to represent the 'item' or 'props' object passed by Recharts.
interface RechartsTooltipFormatterProps {
  payload?: DataPointForHistogram; // The actual data object for the hovered bar/point. This can be undefined.
  label?: string; // The label for the tooltip (often the X-axis value like '时间').
  // Recharts also passes other properties like active, value (same as first arg), name (same as second arg), etc.
  // We only define what we use to keep it simple and fix the error.
}
            ...
            {data.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data.map(d => ({ name: d['时间'], value: d['涨跌幅(%)'] }))} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Tooltip 
                    formatter={(value: number, name: string, itemProps: RechartsTooltipFormatterProps) => {
                      const displayName = itemProps.label ?? itemProps.payload?.name ?? 'N/A';
                      return [formatPercentage(value), `涨跌幅 (${displayName})`];
                    }}
                    contentStyle={{ backgroundColor: '#2D3748', border: '1px solid #4A5568', borderRadius: '0.5rem' }} 
                    labelStyle={{ color: '#E2E8F0' }}
                  />
