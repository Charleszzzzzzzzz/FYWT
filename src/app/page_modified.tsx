...
// Define MarketData interface (already present)
...
// Define interfaces for the Tooltip formatter props
interface HistogramPayload {
  name: string; // This corresponds to '时间' from the original data, mapped to 'name'
  value: number; // This corresponds to '涨跌幅(%)' from the original data, mapped to 'value'
}

interface HistogramTooltipProps {
  payload: HistogramPayload;
  // Add other properties from Recharts Tooltip's 'props' if needed, e.g., label, active
  // For this specific formatter, only payload.name is used from props.payload
  // The 'name' parameter in the formatter function itself is the dataKey of the Bar ('value' in this case)
  // The 'value' parameter in the formatter function is the actual value of that dataKey for the hovered item.
}
          ...
          {data.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <Tooltip 
                  formatter={formatPercentage} // This Tooltip doesn't use the problematic structure
                  contentStyle={{ backgroundColor: '#2D3748', border: '1px solid #4A5568', borderRadius: '0.5rem' }} 
                  labelStyle={{ color: '#E2E8F0' }}
                />
                <Legend wrapperStyle={{ color: '#E2E8F0' }}/>
            ...
            {data.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data.map(d => ({ name: d['时间'], value: d['涨跌幅(%)'] }))} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Tooltip 
                    formatter={(value: number, name: string, props: HistogramTooltipProps) => [formatPercentage(value), `涨跌幅 (${props.payload.name})`]}
                    contentStyle={{ backgroundColor: '#2D3748', border: '1px solid #4A5568', borderRadius: '0.5rem' }} 
                    labelStyle={{ color: '#E2E8F0' }}
                  />
